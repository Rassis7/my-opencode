...
description: Plan and execute deep research, literature reviews, due diligence, competitive analysis, policy comparison, market mapping, or investigative reports in OpenCode with source-verifiable claims, local ledgers, and optional delegation. Use when the user asks for a high-quality report, wants citations or auditable evidence, wants research process files written locally, or needs help managing context-window limits in an OpenCode session.
---
...
- Install this skill in an OpenCode-compatible skills directory.
- Keep the research ledgers in the active project workspace rather than inside the skill folder.
- If OpenCode exposes agent delegation in the current environment, use it only for bounded slices. If not, keep the exact same file-backed workflow as a single orchestrated run.
...
- Record the scope and stop conditions before searching. For a starter layout, load `references/project-layout.md`.
...
- Update these files every round so a later agent can resume without replaying chat. For copy-paste starter files, load `references/templates.md`.

### 3. Decide the delegation pattern

- Only use OpenCode delegation or subagents when the user explicitly asks for multi-agent or delegation work.
...
- Keep the PM/orchestrator role local whenever possible so synthesis and quality control stay centralized. For role options and handoff contracts, load `references/delegation-patterns.md`.
...
- Record enough metadata that another reviewer can reopen the source later. For evidence rules and claim classes, load `references/evidence-standards.md`.
...
- Prefer direct, dated language: "As of 2026-07-02..." over relative timing.
...
- `references/platform-notes.md` -> OpenCode specific installation and usage notes.
